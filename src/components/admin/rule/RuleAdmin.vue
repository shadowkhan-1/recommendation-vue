  ...
  <div class="home">
    ...
    <div style="float:right">
      <el-button
        ...
        type="primary"
        icon="el-icon-plus"
        @click="adduser()" round>添加管理员</el-button>
    </div>
    <el-table
        ...
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            ...
            <div slot="reference" class="name-wrapper">
              <el-tag size="medium">{{ scope.row.rule === '1' ? '管理员' : '普通用户'}}</el-tag>
            </div>
          </el-popover>
        </template>
      </el-table-column>
    ...
    <el-dialog title="用户信息" :visible.sync="dialogFormVisible" v-if="dialogFormVisible">
      <el-form :model="form" ref="usertable" label-position="left">
        ...
        <el-form-item prop="rule" label="权限控制" :label-width="formLabelWidth">
          <div>
            <el-radio v-model="form.rule" label="0" border>普通用户</el-radio>
            <el-radio v-model="form.rule" label="1" border>管理员</el-radio>
          </div>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete (row) {
      ...
      if (row.isDeleted === '0') {
        this.$axios
          .get('/user/delete/todelete/' + modifyBy + '/' + row.userId, {
          }).then(resp => {
            if (resp.data.code === 'SUCCESS') {
              row.isDeleted = '1'
              this.$alert(resp.data.message, '提示', {
        ...
        this.$axios
          .get('/user/delete/rollback/' + modifyBy + '/' + row.userId, {
          }).then(resp => {
            if (resp.data.code === 'SUCCESS') {
              row.isDeleted = '0'
              this.$alert(resp.data.message, '提示', {
